                                ...
                                <el-collapse-item name="1" v-if="showCollapse(commonProps)"
                                    ...
                                    <template v-for="(editorName, propName) in commonProps">
                                        <component v-if="hasPropEditor(propName, editorName)"
                                                   ...
                                                   @change="componentChange($event, optionModel)"></component>
                                    </template>
                                </el-collapse-item>
                                ...
                                <el-collapse-item name="2" v-if="showCollapse(advProps)"
                                    ...
                                    <template v-for="(editorName, propName) in advProps">
                                        <component v-if="hasPropEditor(propName, editorName)"
                                                   :is="getPropEditor(propName, editorName)"
                                                   :designer="designer" :selected-widget="selectedWidget"
                                                   :option-model="optionModel" @change="componentChange($event, optionModel)"></component>
                                    </template>
                                </el-collapse-item>
                                ...
                                <el-collapse-item name="2" v-if="showCollapse(advProps)"
                                    ...
                                    <template v-for="(editorName, propName) in advProps">
                                        <component v-if="hasPropEditor(propName, editorName)"
                                                   ...
                                                   :designer="designer" :selected-widget="selectedWidget"
                                                   :option-model="optionModel"
                                                   @change="componentChange($event, optionModel)"
                                        ></component>
                                    </template>
                                </el-collapse-item>
...
export default {
    ...
    computed: {
        optionModel: {
            get() {
                console.log('this.selectedWidget.options', this.selectedWidget.options)
                return this.selectedWidget.options
            },

            set(newValue) {
                console.log('--======ppppset', newValue)
    ...
    watch: {
        ...
        'selectedWidget.options': {  //组件属性变动后，立即保存表单JSON！！
            deep: true,
            handler() {
                console.log('this.designer.saveCurrentHistoryStep()', this.designer.saveCurrentHistoryStep())
    ...
    created() {
        ...
        console.log('advProps', this.advProps)
    ...
    methods: {
        ...
        hasPropEditor(propName, editorName) {
            ...
            if (propName.indexOf('-') <= -1) {
                ...
                if (propertyRegistered(uniquePropName)) {
                    console.log('hasPropEditor-propName', propName)
                    return false
                }
            }
        ...
        getPropEditor(propName, editorName) {
            // console.log('this.selectedWidget.type', propName, this.selectedWidget.type, this.$options.components)
            let originalPropName = propName.replace(this.selectedWidget.type + '-', '')  //去掉组件名称前缀-，如果有的话！！
